...
const RETENTION: [string, ReactNode][] = [
  ...
  [
    '계정 기록',
    '탈퇴하면 이메일 주소를 되돌릴 수 없는 값으로 바꾸고 구글 계정 식별값을 지웁니다. 서비스 사이에 주고받은 처리 기록에 남은 이메일 · 닉네임은 아래 「서비스 운영 기록」의 기준에 따라 지웁니다. 그 뒤 남는 계정 번호와 로그인 기록(시각, 브라우저 정보)은 이용자를 알아볼 수 없는 상태로 부정 이용 확인을 위해 보관합니다.',
  ],
  ['이메일 인증 코드', '인증을 마치거나 유효 시간이 지나면 삭제합니다.'],
  ['로그인 유지 정보', '로그아웃하거나 유효 기간이 지나면 삭제합니다.'],
  ['웹 서버 접속 기록', 'IP 주소, 브라우저 정보, 접속 일시, 요청 주소 — 14일 뒤 삭제합니다.'],
  ['서비스 운영 기록', '오류와 처리 과정을 남긴 기록, 서비스 사이에 주고받은 처리 기록 — 30일 뒤 삭제합니다.'],
];
